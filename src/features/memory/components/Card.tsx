...
import correctAudio from "../assets/correct.ogg";
import flipAudio from "../assets/flip.wav"
import nopeAudio from "../assets/nope.wav"
...
export default function Card(props: P01955) {
    ...
    function showCard() {
        ...
        audio.src = flipAudio
        audio.volume = 0.1
        if (state.activeCards.length == 0) fn.flipOver({ index: props.index, label: props.card.label })

        else if (state.activeCards.length == 1) {
            if (state.activeCards[0].label == props.card.label) {
                audio.src = correctAudio
                fn.correct(props.card.label)
                if (state.matches.size == state.gameSize * 5) 
                    fn.win()
            }
            else {
                fn.flipOver({ index: props.index, label: props.card.label })
                setTimeout(() => {
                    fn.clearActiveCards()
                }, 1000)
            }
